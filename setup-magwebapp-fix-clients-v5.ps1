# ------------------------------------------------------------
# setup-magwebapp-fix-clients-v5.ps1  (REV 2025-09-13)
# ------------------------------------------------------------
# - Corrige EPERM do Prisma (limpa .prisma\client)
...
# - Ajusta POST /clients (fallbacks + logs do Prisma)
# - Atualiza MagApi no MyDocuments (Mag-Boot envia phone/document/address)
# ------------------------------------------------------------
...
function Run-Migrations {
  ...
  try {
    ...
    $name = "setup_regex_v5_$(Get-Date -Format 'yyyyMMddHHmm')"
...
'@
  ...
  if ($js -match 'app\.listen\s*\(') {
    ...
    $patched = $js.TrimEnd() + "`r`n`r`n// inserted by setup v5`r`n" + $block
  }
...
# --------------- MagApi.psm1 (envia phone/document/address) ---------------
function Fix-MagApi {
...
function Ensure-Client {
  ...
  if ($found.Count) { $found[0] } else { (GetJson "/api/v1/clients" -Method POST -Body @{
    tenantId=$Tenant; email=$email; name=$name; phone="11999990000"; document="DOC-TESTE-API"; address="Rua API, 123"
  }).data }
}
...
function Mag-Boot {
  ...
  if (-not $c) {
    $c = (GetJson "/api/v1/clients" -Method POST -Body @{
      tenantId = $Tenant
      name     = "Cliente Teste"
      email    = "cliente.teste@example.com"
      phone    = "11999990000"
      document = "DOC-TESTE-001"
...
try {
  ...
  Write-Host "`n✅ Setup v5 concluído!" -ForegroundColor White
