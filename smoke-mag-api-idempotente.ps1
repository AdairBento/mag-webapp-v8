# ===== (opcional) liberar execução só nesta sessão =====
...
Set-Content -Path .\smoke-mag-api-check.ps1 -Value $script -Encoding UTF8

# ===== parâmetros (ajuste se quiser) =====
$Base   = 'http://localhost:3000'
$Tenant = '022d0c59-4363-4993-a485-9adf29719824'
$Email  = 'joao.silva+smoke@example.com'
$Plate  = 'SMK1D2'
$From   = '2025-09-24'
$To     = '2025-09-27'

# (opcional) ping rápido
try { Invoke-RestMethod -Uri "$Base/internal/health" -TimeoutSec 3 | Out-Null } catch { Write-Host "⚠️  API pode não estar respondendo em $Base" -ForegroundColor Yellow }

# ===== executar =====
.\smoke-mag-api-check.ps1 -Base $Base -Tenant $Tenant -Email $Email -Plate $Plate -From $From -To $To
Write-Host ("[DEBUG] From={0} To={1}" -f $PSBoundParameters["From"], $PSBoundParameters["To"]) -ForegroundColor DarkCyan
